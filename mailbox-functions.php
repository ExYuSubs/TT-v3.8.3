...
function submenu($pages, $default, $prefix='', $class='submenu', $space='false')
{
 ...
 else $menu = "<table border='0' align='center' cellpadding='5' cellspacing='5' class='table table-bordered'><tr>";
 $arr = explode(',', $pages);
 if ($prefix)
 {
...
function th($x, $field=0, $style='', $col=1)
{
 if (is_string($field))
 {
   if (isset($_GET['sort']) && $field != ($_GET['sort'] ?? null)) $x = "<a href=\"".add_get('sort',$field)."\">$x</a>";
 ...
 if ($style)return "<th style=\"ttable_head\">$x</th>";
 return "<th style=\"ttable_head\">$x</th>";
}
...
function th_right($x, $noesc=0, $style='', $col=1, $field='')  { return th($x, $noesc, 'text-align:left;'.$style, $col, $field); }
...
function pager2($rpp, $count, $opts = array())
{
 ...
 $as = "<img src=\"images/pager_prev.png\" border=\"0\" title=\"".T_("PREVIOUS_PAGE")."\">";
 
 if ($page >= 1) $pager .= '<a href="'.$href.($page - 1).'">'.$as.'</a>';
 else $pager .= $as;
 
 $pager .= "<img src=\"images/pager_middle.png\" border=\"0\">";
 
 $as = "<img src=\"images/pager_next.png\" border=\"0\" title=\"".T_("NEXT_PAGE")."\">";
