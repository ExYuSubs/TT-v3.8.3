...
while ($row = mysqli_fetch_array($res)) {
    $dynamicKey = "c{$row['id']}";
    if (!empty($_GET[$dynamicKey] ?? null)) { // Optional: Replace with `isset()` if 0 is valid
        $wherecatina[] = $row["id"];
        $addparam .= "{$dynamicKey}=1&amp;";
        $thisurl .= "{$dynamicKey}=1&amp;";
    }
}
...
if ($sortKey || $orderKey) {
    switch ($sortKey) {
        ...
        case 'times_completed':
            $sort = "torrents.times_completed";
            $addparam .= "sort=times_completed&amp;";
            break;
        case 'seeders':
            $sort = "torrents.seeders";
            $addparam .= "sort=seeders&amp;";
            break;
        case 'leechers':
            $sort = "torrents.leechers";
            $addparam .= "sort=leechers&amp;";
            break;
        case 'comments':
            $sort = "torrents.comments";
...
while ($cat = mysqli_fetch_assoc($cats)) {
    ...
    if (isset($cat["id"], $cat["parent_cat"], $cat["name"])) {
        $checked = (in_array($cat["id"], $wherecatina) || ($_GET["cat"] ?? null) == $cat["id"]) ? "checked='checked' " : "";
        $catId = htmlspecialchars($cat["id"]);
        $parentCat = htmlspecialchars($cat["parent_cat"]);
        $catName = htmlspecialchars($cat["name"]);
    
        print("<td style=\"padding-bottom: 2px;padding-left: 2px\">
            <a href='torrents.php?cat={$catId}'>{$parentCat} - {$catName}</a> 
            <input name='c{$catId}' type=\"checkbox\" {$checked} value='1' />
            </td>\n");
        $i++;
    }
...
$page = $_GET["page"] ?? null;
if ($page && is_valid_id($page)) {
    $thisurl .= "page=" . urlencode($page) . "&amp;";
}
