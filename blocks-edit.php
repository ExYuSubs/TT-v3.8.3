...
if (isset($_POST["addnew"]) && is_array($_POST["addnew"]) && count($_POST["addnew"])) {
    foreach ($_POST["addnew"] as $addthis) {
        ...
        SQL_Query_exec("INSERT INTO blocks (named, name, description, position, enabled, sort) VALUES ($wantedname, $name, $description, 'left', 0, 0)")
        or ((mysqli_errno($GLOBALS["DBconnector"]) == 1062) 
            ? show_error_msg(T_("ERROR"), "Sorry, this block is in database already!", 1) 
            : show_error_msg(T_("ERROR"), "Database Query failed: " . mysqli_error($GLOBALS["DBconnector"]))
        );
...
// == permanent delete
if (isset($_POST["deletepermanent"]) && is_array($_POST["deletepermanent"]) && count($_POST["deletepermanent"])) {
    foreach ($_POST["deletepermanent"] as $delpthis) {
        $filePath = "blocks/" . $delpthis;
...
if (isset($_REQUEST["edit"]) && $_REQUEST["edit"] == "true")
{
	...
	// == move to left
	if (isset($_GET["left"]) && is_valid_id($_GET["left"])){
	...
	// == move to center
	if (isset($_GET["middle"]) && is_valid_id($_GET["middle"])){
	...
	// == move to right
	if (isset($_GET["right"]) && is_valid_id($_GET["right"])){
	...
	// == move upper
	if (isset($_GET["up"]) && is_valid_id($_GET["up"])){
	...
	// == move lower
	if (isset($_GET["down"]) && is_valid_id($_GET["down"])){
	...
	if (!isset($_GET["up"]) && !isset($_GET["down"]) && !isset($_GET["right"]) && !isset($_GET["left"]) && !isset($_GET["middle"])){
