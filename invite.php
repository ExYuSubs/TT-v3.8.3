...
if ($_GET["take"]) {
	...
	if (!$ret) {
		...
		while (mysqli_errno($GLOBALS["DBconnector"]) == 1062) {
			$username = "invite_".mksecret(20);
				$ret = SQL_Query_exec("INSERT INTO users (username, secret, email, status, invited_by, added, stylesheet, uploaded, downloaded, language, invites) VALUES (".
	implode(",", array_map("sqlesc", array($username, $secret, $email, 'pending', $CURUSER["id"]))) . ",'" . get_date_time() . "', $site_config[default_theme], $site_config[new_member_upload_ratio], 0, $site_config[default_language], $site_config[new_member_invites])");
		}
