...
if (!$site_config["MEMBERSONLY"] || $CURUSER) {
    ...
    if (strtoupper(substr(PHP_OS, 0, 3)) == "WIN") {
        ...
        if ($reguptime) {
            if (preg_match("/up\s+(?:(\d+)\s+days?,\s+)?(?:(\d+):(\d+))?(?:\s+(\d+)\s+min)?,/", $reguptime, $uptimeMatches)) {
                $days = isset($uptimeMatches[1]) ? $uptimeMatches[1] : 0;
                $hours = isset($uptimeMatches[2]) ? $uptimeMatches[2] : 0;
                $minutes = isset($uptimeMatches[4]) ? $uptimeMatches[4] : (isset($uptimeMatches[3]) ? $uptimeMatches[3] : 0);
